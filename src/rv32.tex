...
The global memory order is defined in terms of the primitive load and/or store operation(s) generated by each memory instruction.
...
\begin{table}[h!]
  ...
  \end{tabular}
  \caption{\textcolor{red}{THIS MAY CHANGE!} Mapping of instructions into memory operations, for the purposes of describing the memory model.  ``{\tt.sc}'' is a synonym for ``{\tt.aqrl}''.}
...
Future ISA extensions, such as the ``V'' or ``T'' extensions, may contain instructions which generate memory operations in other extension-defined ways.
...
\begin{commentary}
  Although the ISA does not currently contain {\tt l\{b|h|w|d\}.aq[rl]} or {\tt s\{b|h|w|d\}.[aq]rl} instructions, the RVWMO model itself supports them as if they were in fact natively available, for the sake of forwards compatibility with their potential future addition into the ISA.  In the meantime, they are supported as assembler pseudoinstructions.   Pending any future addition of these instructions to the ISA, these pseudoinstructions will assemble into sufficiently-strong sequences of available opcodes, via mappings such as the one in Section~\ref{sec:porting}.
\end{commentary}

\subsubsection*{Dependencies}
The definition of the RVWMO memory model depends in part on the notion of a syntactic dependency, defined as follows.

%In the context of defining dependencies, a ``register'' refers either to an entire general-purpose register or a CSR.
A register $r$ other than {\tt x0} is a {\em source register} for an instruction $i$ if either of the following hold:
\begin{itemize}
...
A register $r$ other than {\tt x0} is a {\em destination register} for an instruction $i$ if either of the following hold:
\begin{itemize}
  \item $i$ sets {\em rd} to $r$
  \item $i$ is a CSR instruction that sets {\em csr} to $r$, unless $i$ is {\tt csrrs} or {\tt csrrc} and {\em rs1} is set to {\tt x0} or $i$ is {\tt csrrsi} or {\tt csrrci} and uimm[4:0] is zero.
\end{itemize}

Each instruction {\em carries a dependency} from its source register(s) to its destination register(s), with the exception of memory instructions and indirect jumps.

\textcolor{red}{DL: does CSRRW carry a dependency from {\em csr} to {\em csr} or from {\em rs1} to {\em rd}?  Or just from {\em csr} to {\em rd} and from {\em rs1} to {\em csr}?}

A memory operation $m$ {\em produces} a register $r$ if $r$ is the destination register for the instruction that generates $m$ and $m$ is either a load operation or the store operation generated by a store-conditional instruction.

\begin{commentary}
  ISA extentions other than the standard M, A, F, and D extensions may introduce their own variations or extensions on the definitions above.
\end{commentary}

Instruction $j$ has a {\em syntactic dependency} on instruction $i$ via register $r$ if either of the following hold:
\begin{itemize}
  \item $r$ is a destination register for $i$, $r$ is a source register for $j$, and no instruction program-ordered between $i$ and $j$ has $r$ as a destination register.
...
$b$ has a {\em syntactic address dependency} on $a$ if $a$ produces a destination register of $i$, $r$ is the address source register for $j$, and $j$ has a syntactic dependency $i$ via $r$
...
\begin{itemize}
  ...
  \item $a$ produces a destination register of $i$, $r$ is the data source register for $j$, and $j$ has a syntactic dependency $i$ via $r$
\end{itemize}

$b$ has a {\em syntactic control dependency} on $a$ if $a$ produces a destination register of $i$, there is an instruction $m$ program-ordered between $i$ and $j$ such that $m$ is a branch or indirect jump, and $m$ has a syntactic dependency on $i$

\begin{commentary}
  The intention of the dependency rules is as follows:
  \begin{itemize}
    \item The artificial data dependency inside {\tt amoswap} and those {\tt lr}/{\tt sc} pairs that would not otherwise be considered to have a data dependency is intentional.
    ...
    \item Dependencies through implicit CSR updates (e.g., to {\tt fflags} or to {\tt instret}) are not respected by the memory consistency model.
  \end{itemize}
\end{commentary}
...
\begin{itemize}
  \item All {\tt l\{b|h|w|d|r\}} instructions behave as if all load memory operations have an RCpc annotation (or stronger)
  \item All {\tt s\{b|h|w|d|c\}} instructions behave as if all store memory operations have an RCpc annotation (or stronger)
  \item All AMO instructions behave as if AMOs have an SC annotation
\end{itemize}
