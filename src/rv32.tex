...
The base RISC-V ISA provides a {\tt fence} instruction for this purpose, described in Section~\ref{sec:fence}, while the optional ``A'' atomics extension additionally defines load-reserved/store-conditional and atomic read-modify-write instructions.
...
Each aligned memory instruction other than an unsuccessful {\tt sc} instruction gives rise to exactly one memory operation.
...
\begin{itemize}
  ...
  \item $i$ is a CSR instruction, and in the opcode of $i$, {\em csr} is set to $r$, unless $i$ is {\tt csrrw} or {\tt csrrwi} and {\em rd} is set to {\tt x0}
...
\begin{itemize}
  ...
  \item $i$ is a CSR instruction, and in the opcode of $i$, {\em csr} is set to $r$, unless $i$ is {\tt csrrs} or {\tt csrrc} and {\em rs1} is set to {\tt x0} or $i$ is {\tt csrrsi} or {\tt csrrci} and uimm[4:0] is set to zero.
...
\newcommand{\ppoamoforward}{$a$ is generated by an AMO or {\tt sc} instruction, $b$ is a load, and $b$ returns a value written by $a$}
