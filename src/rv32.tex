...
In the memory model literature, the term ``RCpc'' stands for release consistency with processor-consistent synchronization operations, the term ``RCsc'' stands for release consistency with sequentially-consistent synchronization operations, and the term ``SC'' simply stands for sequentially-consistent.
...
Instructions between instructions $i$ and $j$ carry a dependency from register $r$ to register $s$ if any of the following hold:
\begin{enumerate}
	\item there are no instructions between $i$ and $j$ in the program order that modify $s$, and $r$ is the same register as $s$
	\item there are an instruction $i'$ between $i$ and $j$ in the program order and registers $r'$, $s'$ such that instructions between $i$ and $i'$ carry a dependency from $r$ to $r'$, 
	 $i'$ {\em carries a dependency} from $r'$ to $s'$, and instructions between $i'$ and $j$ carry a dependency from $r'$ to $s$.
\end{enumerate}
The question of whether a given instruction carries a dependency from certain input registers to certain output registers is specified as part of the definition of each instruction; however, we also provide a summary here.
...
For all instructions which read {\em rs1} and {\em rs2} and write {\em rd}, {\em rd} carries a dependency from both {\em rs1} and {\em rs2}, with the exception of {\tt amoswap}, {\tt amoadd}, {\tt amoand}, {\tt amoor}, {\tt amoxor}, {\tt amomax}, {\tt amomin}, and {\tt sc}. With the exception of {\tt amoswap} and {\tt sc}, all of these carry a dependency from {\em rs2} to {\em rd}.

Dependencies through CSRs are tracked at bit granularity, rather than at register granularity.
CSR instructions carry a dependence from all bits in {\em csr} to {\em rd}, from {\em rs1} to all bits in {\em csr}, and from each bit in {\em csr} to the same bit in {\em csr}, except that {\tt csrrw} does not carry any dependencies from {\em csr} to {\em csr}.
...
\begin{itemize}
\item $b$ has a syntactic address dependency on $a$ if the register read by $b$ which is determines the address accessed by $b$ has a syntactic dependency on $a$
\item $b$ has a syntactic data dependency on $a$ if $b$ is a store and the register read by $b$ which determines the data being written by $b$ has a syntactic dependency on $a$, or if $a$ and $b$ are paired
...
\begin{commentary}
  ...
  \begin{itemize}
    ...
    \item Accesses to CSR bits which alias with other CSR bits are considered accesses to both (or all) aliases.
    ...
    \item Instructions which interact with CSRs that accumulate conditions (e.g., the overflow condition {\tt OZ} in the ``F'' CSR {\tt fcsr}) are considered to read the old value and write the new value, so that the new value depends on the previous writer(s) and the current instruction.
  \end{itemize}
