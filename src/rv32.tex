...
\begin{table}[h!]
  ...
  \begin{tabular}{|l|l|}
    \hline
    RISC-V Instruction & Memory Operations and Ordering Annotations \\
    \hline
    \hline
    \tt l\{b|h|w|d\}    & load$^*$               \\
    \tt s\{b|h|w|d\}    & store$^*$              \\
...
Each aligned load or store instruction gives rise to exactly one memory operation, which executes atomically: a load or store can never be observed in a partially-complete state.
A misaligned load or store instruction may be decomposed into a set of component load or store memory operations at any granularity.
...
AMOs give rise to exactly two memory operations: one load and one store.  These operations are said to be {\em paired}.  The store is program-ordered after the load.
...
Load and store operations may also carry ordering annotations.
In strictly increasing order of strength, these annotations are ``RCpc'', ``RCsc'', and ``SC''.
In the memory model literature, the term ``RCpc'' stands for release consistency with processor-consistent synchronization operations, the term ``RCsc'' stands for release consistency with sequentially-consistent synchronization operations, and the term ``SC'' stands for sequentially-consistent.
Under RC, load and store synchronization operations are known as ``load-acquire'' and ``store-release'' operations, respectively.
The precise semantics of these annotations as they apply to RVWMO are described by the memory model rules below.

\begin{commentary}
  Although the ISA does not currently contain {\tt l\{b|h|w|d\}.aq[rl]} or {\tt s\{b|h|w|d\}.[aq]rl} instructions, the RVWMO model itself supports them as if they were in fact natively available, for the sake of forwards compatibility with their potential future addition into the ISA.  In the meantime, they are supported as assembler pseudoinstructions.  These pseudoinstructions generally assemble per the fence-based mappings of Section~\ref{sec:porting} pending any future addition of these instructions to the ISA.
\end{commentary}

\subsubsection*{Dependencies}
The definition of the RVWMO memory model depends in part on the notion of a syntactic dependency, defined as follows.

In the context of defining dependencies, a ``register'' refers either to an entire general-purpose register or a single bit of a CSR.

Instructions between instructions $i$ and $j$ carry a dependency from register $r$ to register $s$ if either of the following hold:
\begin{enumerate}
\item $r$ is the same register as $s$ and no instruction program-ordered between $i$ and $j$ in the program order writes $s$, or 
\item there are registers $r'$, $s'$ and an instruction $i'$ program-ordered between $i$ and $j$ such that:
  \begin{enumerate}
  \item instructions between $i$ and $i'$ carry a dependency from $r$ to $r'$,
  \item $i'$ {\em carries a dependency} from $r'$ to $s'$, and
  \item instructions between $i'$ and $j$ carry a dependency from $r'$ to $s$.
  \end{enumerate}
\end{enumerate}
Whether a given instruction carries a dependency from certain source registers to certain destination registers is specified as part of the definition of each instruction; however, we also provide a summary here.
All instructions reading {\em rs1} and writing {\em rd}, carry a dependency from {\em rs1} to {\em rd}, with the exception of {\tt lb}, {\tt lh}, {\tt lw}, {\tt ld}, {\tt lr}, and {\tt jalr}.
All instructions reading {\em rs1} and {\em rs2} and writing {\em rd}, carry dependencies from {\em rs1} to {\em rd} and from {\em rs2} to {\em rd}, with the exception of {\tt amoswap}, {\tt amoadd}, {\tt amoand}, {\tt amoor}, {\tt amoxor}, {\tt amomax}, {\tt amomin}, and {\tt sc}, which with the exception of {\tt amoswap} and {\tt sc}, carry a dependency from {\em rs2} to {\em rd}.

Dependencies through CSRs are tracked at bit granularity, rather than at register granularity.
All CSR instructions carry a dependency from all bits in {\em csr} to {\em rd}.
CSR instructions \texttt{csrrw}, \texttt{csrrs}, and \texttt{csrrc} carry a dependency from {\em rs1} to all bits in {\em csr}.
CSR instructions \texttt{csrrs} and \texttt{csrrc} carry a dependency from each bit in {\em csr} to the same bit in {\em csr}.
CSR instructions \texttt{csrrsi} and \texttt{csrrci}, for each bit $b$, where $b \ge 5$ or for which field \emph{rs1}$[b]$ is zero, carry a dependency from bit $b$ in {\em csr} to bit $b$ in {\em csr}.
Finally, some instructions implicitly read and/or write CSRs as part of their normal operation; the specification of how these instructions carry dependencies to/from these implicitly-accessed CSRs and between CSRs and general-purpose registers is \textcolor{red}{(TODO)} defined as part of the specification of each instruction individually.

\textcolor{red}{PL: The CSR instruction dependency-carrying specification above may be too detailed for this section.}

A memory operation $m$ is said to read a register $r$ if the instruction that generates $m$ reads $r$, unless $m$ is the load operation generated by an AMO and $r$ is the data source register {\em rs2}.
...
With that, a register $s$ read by instruction $i$ has a syntactic dependency on memory operation $a$ if $a$ precedes $i$ in program order, $s$ is neither {\tt x0} nor {\tt pc}, $a$ writes a register $r$, and the instructions between $a$ and $i$ carry a dependency from $r$ to $s$.

Dependencies between memory operations are defined as follows: for any two memory operations $a$ and $b$,
\begin{itemize}
\item $b$ has a syntactic address dependency on $a$ if a register $r$ read by $b$ has a syntactic dependency on $a$ and $r$ determines the address accessed by $b$ 
\item $b$ has a syntactic data dependency on $a$ if $b$ is a store and either $a$ and $b$ are paired or a register $r$ read by $b$ has a syntactic dependency on $a$ and $r$ determines the data being stored by $b$ 
\item $b$ has a syntactic control dependency on $a$ if there is an instruction $m$ between $a$ and $b$ in program order where a register $s$ read by $m$ has a syntactic dependency on $a$ and either
  \begin{itemize}
  \item $m$ is a branch and $s$ is checked as part of the condition of $m$, or
  \item $m$ is an indirect jump and $s$ is used to calculate the jump address of $m$
  \end{itemize}
\end{itemize}
...
\begin{commentary}
  ...
  \begin{itemize}
    \item There is a data dependency from each paired load operation to the corresponding paired store operation, even for {\tt amoswap} and those {\tt lr}/{\tt sc} pairs that would not otherwise be considered to have data dependencies.
    \item A dependency can originate and be used to enforce ordering from the {\em rd} destination register of a successful {\tt sc} instruction: subsequent instructions having a syntactic dependency on the success destination of an {\tt sc} operation are not performed until the store operation generated by the {\tt sc} has completed.
    \item A dependency cannot originate or be used to enforce ordering from the {\em rd} destination register of an unsuccessful {\tt sc} instruction.
    \item Accesses to CSR bits aliased to other CSR bits are considered accesses to both (or all) aliases.
    \item Dependencies through CSRs are tracked at bit granularity to enable individual bits within a CSR to operate independently.
    \item Instructions interacting with CSRs that accumulate conditions (e.g., the overflow condition {\tt OZ} in the ``F'' CSR {\tt fcsr}) are considered to read the old value and write the new value, so that the new value depends on the previous writer(s) and the current instruction.  This is for memory-model purposes only and does not imply that such instructions have to be executed in program order.
  \end{itemize}
  \textcolor{red}{(DL: This blurb may move into the explanatory material appendix at some point, but for now as we're actively discussing this I'll keep it here)}
\end{commentary}
...
\newcommand{\ppoacquire}{$a$ is a load with RCpc annotation or stronger}
\newcommand{\pporelease}{$b$ is a store with RCpc annotation or stronger}
\newcommand{\ppostrongacqrel}{$a$ and $b$ both have RCsc annotations or stronger}
\newcommand{\ppoamoload}{$b$ has an SC annotation}
\newcommand{\ppoamostore}{$a$ has an SC annotation}
...
%\newcommand{\ppoloadtoacq}{$a$ is a paired store, $b$ is a load-acquire, $a$ and $b$ are accesses to overlapping memory accesses, and there is no other store to address between the store paired with $a$ and $b$}
...
\newcommand{\ppoldstld}{$b$ is a load, and there exists some store $m$ between $a$ and $b$ in program order such that $m$ has an address or data dependency on $a$, and $b$ reads a value written by $m$}
...
\newcommand{\loadvalueaxiom}{
  Each byte of each load returns the corresponding byte written by whichever of the following two stores comes later in the global memory order:
  \begin{enumerate}
...
\textcolor{red}{PL: The above is somewhat ambiguous; it is not clear
  to what orders ``latest'' applies.  Fixing it leads to somewhat
  clumsy wording.  It is these niggling ambiguities in English prose
  that lead to my preference for backing up the English with
...
Finally, they also imply that all AMO instructions are fully-fenced; nothing is reordered past an AMO.
