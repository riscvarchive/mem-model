...
An AMO gives rise to a memory operation which is both a load operation and a store operation simultaneously.
Unsuccessful {\tt sc} instructions do not generate memory operations.

A misaligned load or store instruction may be decomposed into a set of component load or store memory operations of any granularity, unless misaligned atomics are supported as described in Section~\ref{sec:misalignedamo}.
...
The complete list of conditions determining the success or failure of store conditional instructions is defined in the ``A'' extension specification.
...
\begin{commentary}
  %{\tt sc.aq} and {\tt lr.rl} are discouraged, as ``acquire'' semantics are generally said to apply only to load operations, and ``release'' semantics only to store operations.
...
In the context of defining dependencies, a ``register'' refers either to an entire general-purpose register, some portion of a CSR, or an entire CSR.  The granularity at which dependencies are tracked through CSRs is specific to each CSR.
For a complete description of the granularity at which dependencies are tracked through each CSR, see Appendix~\ref{ch:deps}.
...
This section provides a general definition of all of these terms; however, Appendix~\ref{ch:deps} provides a comprehensive summary of these terms as they apply to each instruction in the ISA.

A register $r$ other than {\tt x0} is a {\em source register} for an instruction $i$ if any of the following hold:
\begin{itemize}
  ...
  \item $r$ is a CSR and an implicit source register for $i$, as defined in the specification of $i$ in Appendix~\ref{ch:deps}
\end{itemize}
...
A register $r$ other than {\tt x0} is a {\em destination register} for an instruction $i$ if any of the following hold:
\begin{itemize}
  ...
  \item $r$ is a CSR and an implicit destination register for $i$, as defined in the specification of $i$ in Appendix~\ref{ch:deps}
\end{itemize}
If a general-purpose register $r$ is a destination register for $i$, then $i$ is said to write to $r$.
If a CSR $r$ is a destination register for $i$, then $i$ will be further specified either to write to $r$ or to accumulate into $r$.

Except for memory instructions and indirect jumps, each instruction {\em carries a dependency} from each of its source register(s) to each of its destination register(s).
Memory instructions and indirect jumps do not carry a dependency from any source registers to any destination registers.
\begin{comment}
\begin{commentary}
  The value written into the destination register of a memory instruction or indirect jump is not determined directly from the contents of the source registers.  In the case of memory instructions, the value written into the destination register ultimately comes from the memory system.  In the case of indirect jumps, the value written into the destination register comes from the current {\tt pc}.
\end{commentary}
\end{comment}
...
\begin{itemize}
  ...
  \item $a$ $r$ is a data source register for $j$, and $j$ has a syntactic dependency $i$ via $r$
\end{itemize}
...
\begin{commentary}
  ...
  \begin{itemize}
    ...
    \item A dependency can originate and be used to enforce ordering from the {\em rd} destination register of a successful {\tt sc} instruction.  A dependency cannot originate or be used to enforce ordering from the {\em rd} destination register of an unsuccessful {\tt sc} instruction.
    \item Accesses to CSR bits aliased to other CSR bits are considered accesses to both (or all) aliases.
    \item Dependencies are not tracked through the {\tt pc} register.
    \item Instructions carry a dependency from every source to every destination register, unless otherwise specified.
  \end{itemize}
\end{commentary}
...
\newcommand{\ppoacquire}{$a$ is a load with an acquire annotation}
\newcommand{\pporelease}{$b$ is a store with a release annotation}
\newcommand{\ppostrongacqrel}{$a$ and $b$ both have RCsc annotations}
\newcommand{\ppoamoload}{$b$ has an SC annotation}
\newcommand{\ppoamostore}{$a$ has an SC annotation}
\newcommand{\ppoamoforward}{$a$ is an AMO, and $b$ is a load which accesses an overlapping memory address}
...
\newcommand{\atomicityaxiom}{If $r$ and $w$ are paired load and store operations generated by aligned atomic instructions, and if $s$ is any store to byte $x$ from which $r$ returns a value, then there can be no store from another hart to byte $x$ following $s$ and preceding $w$ in the global memory order.}
...
\begin{commentary}
  ...
  \begin{enumerate}
    \item On hardware which natively supports misaligned accesses to the address and size in question (e.g., for misaligned accesses within a single cache line): by simply ensuring that only a single memory operation is generated for each such instruction
    \item On hardware which does not natively support misaligned accesses to the address and size in question: by trapping on all instructions (including loads) with that address and size and executing them (via any number of memory operations) inside a mutex that is specific to the given memory address and access size.  AMOs may be emulated by splitting them into separate load and store operations, but all preserved program order rules (e.g., incoming and outgoing syntactic dependencies) must behave as if the AMO is still a single memory operation.
  \end{enumerate}
\end{commentary}
...
These rules render PPO rules \ref{ppo:->st} and \ref{ppo:amoforward}--\ref{ppo:addrpo} redundant.
