...
\begin{figure}[h!]
  \centering
    ...
    \end{tabular}
    & &
    \input{figs/B1+A.pdf_t}
%\input{figs/B1+B.pdf_t} %Another invalid execution
\end{tabular}
...
\subsection{Syntactic Dependencies (Rules~\ref{ppo:addr}--\ref{ppo:pair})}
\label{sec:depspart1}
\begin{tabular}{p{1cm}|p{12cm}}
...
Notably, PPO rules \ref{ppo:addr}--\ref{ppo:pair} are also intentionally designed to respect dependencies that originate from the output of a successful store conditional instruction.
...
For example: a syntactic dependency between two ``F'' instructions via one of the ``accumulating CSRs'' in Appendix~\ref{ch:deps} does {\em not} imply that the two ``F'' instructions must be executed in order.
...
\begin{figure}[h!]
  \centering
  \begin{tabular}{m{.2\linewidth}m{.05\linewidth}m{.2\linewidth}}
  {
    \tt\small
    \begin{tabular}{cl}
      (a) & lw a0, 0(s0)   \\
...
Furthermore, in order to enforce store-release-to-load-acquire ordering, there must be a {\tt fence rw,rw} between the store-release and load-acquire; Table~\ref{tab:armmappings} enforces this by always placing the fence in front of each acquire operation.
...
\begin{itemize}
  \item programmers will use PPO rules \ref{ppo:->st}, \ref{ppo:fence}--\ref{ppo:rcsc}, and \ref{ppo:pair} regularly and actively.
