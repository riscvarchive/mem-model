...
\textcolor{red}{\bf DL: this is new, please check!}
\begin{itemize}
  \item {\tt fence.i}: Conceptually, an instruction fetch behaves as if it were a load to the same address, of the same size, and located anywhere in program order between its original location and the location of the most recent {\tt fence.i} preceding the instruction being fetched in program order.
    This means that instruction caches and/or pipelines that hardware does not keep coherent with normal memory must be invalidated/flushed when a {\tt fence.i} instruction is executed.
  \item {\tt sfence.vma}: Conceptually, each virtual-to-physical address translation for an instruction behaves as if it were a load to the address of the relevant page table entry located anywhere in program order between the associated instruction and the location of the most recent {\tt sfence.vma} preceding the instruction being fetched in program order.
    This means that TLBs and/or other paging caches that hardware does not keep coherent with normal memory must be invalidated when a {\tt sfence.vma} instruction is executed.
...
In the latter case, the {\tt .rl} annotation on the {\tt lr} is redundant and can be omitted.
...
\begin{table}[h!]
  ...
  \begin{tabular}{|l|l|}
    ...
    Load-Reserve              & \tt lr.\{w|d\}  \\
    \hline
    Load-Reserve-Acquire      & \tt lr.\{w|d\}.aq \\
    \hline
    Store                     & \tt s\{b|h|w|d\}  \\
    \hline
    ...
    Store-Conditional         & \tt sc.\{w|d\}  \\
    \hline
    Store-Conditional-Release & \tt sc.\{w|d\}.rl  \\
    \hline
    \tt dmb                   & \tt fence rw,rw \\
    \hline
...
\begin{table}[h!]
  ...
  \begin{tabular}{|l|l|l|}
    \hline
    Linux Construct            & RISC-V Mapping               & Hypothetical Alternative Mapping \\
    \hline
    \tt smp\_load\_acquire()   & \tt l\{b|h|w|d\}; fence r,rw & \tt l\{b|h|w|d\}.aq \\
    \hline
    \tt smp\_store\_release()  & \tt fence.tso; s\{b|h|w|d\}  & \tt fence w,w; s\{b|h|w|d\}.rl\\
    \hline
  \end{tabular}
  ...
  \begin{tabular}{|l|l|l|}
    \hline
    Linux Construct            & RISC-V AMO Mapping       & RISC-V LR/SC Mapping\\
    \hline
    \tt atomic\_<op>\_relaxed  & \tt amo<op>.\{w|d\}      & \tt lr.\{w|d\};~~~ <op>; sc.\{w|d\} \\
    \hline
    \tt atomic\_<op>\_acquire  & \tt amo<op>.\{w|d\}.aq   & \tt lr.\{w|d\}.aq; <op>; sc.\{w|d\} \\
    \hline
    \tt atomic\_<op>\_release  & \tt amo<op>.\{w|d\}.rl   & \tt lr.\{w|d\};~~~ <op>; sc.\{w|d\}.rl \\
    \hline
...
\end{figure}
The Linux mappings for {\tt smp\_store\_release()} may seem stronger than necessary, but these are needed to cover some cases in which Linux has slightly stronger requirements than RCpc.
For example, there is a proposal as of the time of writing that Linux should require (a) to be ordered before (e) in Figure~\ref{fig:litmus:lkmm_ll}.
...
~
  ...
  \begin{tabular}{|l|l|l|}
    \hline
    C/C++ Construct                               & RISC-V AMO Mapping       & RISC-V LR/SC Mapping\\
    \hline
