...
Without rule~\ref{ppo:loadtoacq}, (c) would precede (d), but (d) would {\em not} precede (e) due to ``fri; rfi'' behavior (Chapter~\ref{sec:ppo:rdw}).
...
\section{Axiomatic Specification in Alloy}
\label{sec:alloy}

\textcolor{red}{FIXME! OUT OF DATE!}
...
(* Read same write *)
let rsw = rf^-1;rf
(* Load acquire, or stronger  *)
let LD-ACQ = R & (Acq|AcqRel)
(* Store release or stronger *)
