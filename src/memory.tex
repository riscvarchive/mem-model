...
More over, since SC is defined to carry dependencies from its source registers to {\em rd} only when the SC is successful, an unsuccessful SC has no effect on the global memory order.
...
\begin{figure}[h!]
  ...
  \begin{tabular}{m{.4\linewidth}m{.05\linewidth}m{.2\linewidth}}
    ...
    (c) & sw t1,0(s1) & (f) & sw t1, 0(s0)   \\
    \hline
    \multicolumn{4}{c}{Outcome: {\tt a0=1}, {\tt a1=1}}
    \end{tabular}  
    & &
