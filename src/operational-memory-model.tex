...
\begin{itemize}
\item \nameref{omm:hart:fetch}: This transition represents a fetch and decode of a new instruction instance, as a program order successor of a previously fetched instruction instance.
\fixme{we used to also say ``or at the initial fetch address''; although a complete model will have to handle the initial fetch somehow, we never really thought about it so this addition is more to do with how we hacked rmem;
hence I think it's better to omit this detail, especially here in the informal part, where the text is not expected to be precise.}
...
\begin{itemize}
\item[$\circ$] \nameref{omm:hart:initiate_load}: At this point the memory footprint of the load is provisionally known and its individual memory load operations can start being satisfied.
...
\begin{itemize}
\item[$\circ$] \nameref{omm:hart:announce_store_footprint}: At this point the memory footprint of the store is provisionally known.
\item[$\circ$] \nameref{omm:hart:initiate_store}: At this point the memory store operations have their values and program order subsequent memory load operations can be satisfied by forwarding from them.
...
\item \nameref{omm:hart:prop_store}: This propagates a single memory store operation to memory.
...
\paragraph{Instantiate memory store operation values of store instruction}\label{omm:hart:initiate_store}
...
\subsection{Model Limitations}\label{sec:omm:limitations}
\begin{itemize}
...
In particular, the \nameref{omm:hart:fetch} transition does not generate memory load operations, and the shared memory is not involved in the transition.
