...
\begin{itemize}
\item \nameref{omm:early_sc_fail}: This causes the {\em sc} to fail early, either a spontaneous fail, or because there is no program-order previous {\em lr} to pair with.
\item \nameref{omm:paired_sc}: This is an alternative to \nameref{omm:early_sc_fail}. \fixme{after which the {\em sc} will fail only if the atomicity with the paired {\em lr} is violated.}
