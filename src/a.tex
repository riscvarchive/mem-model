...
a misaligned address exception will be generated.  Support for misaligned
atomics is defined in the ``Zam'' extension (Chapter~\ref{sec:zam}).
...
for a single hart.  Informally speaking, an SC can succeed if no accesses from other harts
...
this hart, between the LR and the SC.  The precise atomicity
requirements are detailed in Section~\ref{sec:rvwmo}.
...
exception will be generated.  Support for misaligned atomics is defined in the
``Zam'' extension (Chapter~\ref{sec:zam}).
